...
const resetPassword = asyncHandler(async (req, res) => {
  ...
  if (!patternPassword.test(newPassword)) {
    error = {
      ...error,
      password: [
        "New password must contain one digit from 1 to 9, one lowercase letter, one uppercase letter, one special character, no space, and it must be 8-16 characters long.",
      ],
    };
    isEmpty = true;
  }
  ...
  if (user) {
    if (req.headers.authorization) {
      ...
      res.json({
        ...
        listFollow: listFollowUser.map((item) => item.checkFollow(user)),
      });
    } else {
      res.json({
...
const getFollowList = asyncHandler(async (req, res) => {
  ...
  if (user) {
    ...
    if (req.headers.authorization) {
      ...
      const userCurrent = await User.findOne({ _id: decoded.id });
      res.json({
        users: listFollowUser.map((item) => item.checkFollow(userCurrent)),
        page,
