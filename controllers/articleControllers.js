...
const getArticlesPopular = asyncHandler(async (req, res) => {
  const articles = await Article.find({}).sort({ favoriteCount: -1 }).limit(6);
  if (articles) {
    res.json({ articles });
  } else {
    res.status(404);
...
const updateArticle = asyncHandler(async (req, res) => {
  const { title, description, body, thumbnailUrl, tagList } = req.body;
  ...
  if (isEmpty) {
    ...
    const uniqTags = [...new Set(tagList)].filter(
      (tag) => article && !article.tagList.includes(tag)
    );
    const delTags = article.tagList.filter(
      (tag) => article && ![...new Set(tagList)].includes(tag)
    );
		delTags.forEach(async (tag) => {
    ...
    if (article) {
      ...
      article.tagList = [...new Set(tagList)];
